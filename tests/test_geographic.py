...
from hmc_utils.geographic import (import_way_points, LocationCheck)
...
def test_import_way_points():
    ...
    data_expected = pd.DataFrame(index=np.array(
        [0., 51.35344852, 103.76100595, 280.37591193, 540.1554197, 718.74267821,
         892.77067385, 1121.19537128, 1380.98042693, 1590.32023099, 1813.18621139,
         2002.15956558, 2176.81400435, 2400.01710602, 2519.33685128]))
    data_expected.index.name = "travel_distance"
    data_expected["latitude"] = np.array(
        [32.4, 31.8, 31.09051332, 28.3588147, 24.03366368, 21.10140247,
         18.20281473, 14.38076727, 10.24088837, 7.39676749, 4.39631756, 2.64584774,
         1.5183185, 3.43848364, 4.90171136])
    data_expected["longitude"] = np.array(
        [-17.18, -17.9, -18.49808275, -19.77673541, -20.20678434,
         -19.59538317, -19.30659972, -19.38683736, -18.0310994, -15.97084304,
         -13.75368275, -11.12936662, -8.44639843, -5.25718259, -3.90157281])
    data_expected["heading"] = np.array(
        [225.80012291, 216.01231006, 202.54248937, 185.2219716, 168.92178621,
         174.55887537, 181.17301186, 162.00194422, 144.06214121, 143.38056125,
         123.49090439, 112.61847796, 59.03325232, 42.88156671, 42.88156671])
...
def test_import_way_points_interpolated():
    ...
    data_kml_expected = pd.DataFrame(index=np.array(
    ...
    data_expected = pd.DataFrame(index=np.array(
        [0.0, 2.44157154192, 3.6625019557, 4.88352875138, 6.10465189836]))
    data_expected.index.name = "travel_distance"
    data_expected["heading"] = np.array(
...
def test_location_check():
    location_check = LocationCheck(latitude=52.37, longitude=4.89, distance_deviation_allowed=10)

    out_of_range = location_check.out_of_range(current_latitude=52.31, current_longitude=4.78)
    assert_equal(out_of_range, True)
    assert_almost_equal(location_check.distance, 10.03906752260248)

    out_of_range = location_check.out_of_range(current_latitude=52.36, current_longitude=4.902)
    assert_equal(out_of_range, False)
    assert_equal(location_check.distance, 1.3807122835704022)
